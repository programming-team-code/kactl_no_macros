...
vector<int> mo(vector<pair<int, int>> Q) {
  ...
  sort(begin(s), end(s), [&](int s, int t) { return pair<int, int>(Q[s].first / blk, Q[s].second ^ -(Q[s].first / blk & 1)) < pair<int, int>(Q[t].first / blk, Q[t].second ^ -(Q[t].first / blk & 1)); });
  for (int qi : s) {
    pair<int, int> q = Q[qi];
...
vector<int> moTree(vector<array<int, 2>> Q, vector<vector<int>>& ed, int root = 0) {
  ...
  dfs(root, -1, 0, dfs);
  iota(begin(s), end(s), 0);
  sort(begin(s), end(s), [&](int s, int t) { return pair<int, int>(I[Q[s][0]] / blk, I[Q[s][1]] ^ -(I[Q[s][0]] / blk & 1)) < pair<int, int>(I[Q[t][0]] / blk, I[Q[t][1]] ^ -(I[Q[t][0]] / blk & 1)); });
  for (int qi : s)
