...
template <class P> int extrVertex(vector<P>& poly, P dir) {
  ...
  if (sgn(dir.perp().cross(poly[(0 + 1) % n] - poly[(0) % n])) >= 0 && sgn(dir.perp().cross(poly[(0) % n] - poly[(0 - 1 + n) % n])) < 0) return 0;
  while (lo + 1 < hi) {
    int m = (lo + hi) / 2;
    if (sgn(dir.perp().cross(poly[(m + 1) % n] - poly[(m) % n])) >= 0 && sgn(dir.perp().cross(poly[(m) % n] - poly[(m - 1 + n) % n])) < 0) return m;
    int ls = sgn(dir.perp().cross(poly[(lo + 1) % n] - poly[(lo) % n])), ms = sgn(dir.perp().cross(poly[(m + 1) % n] - poly[(m) % n]));
    (ls < ms || (ls == ms && ls == sgn(dir.perp().cross(poly[(lo) % n] - poly[(m) % n]))) ? hi : lo) = m;
  }
  return lo;
}
template <class P>
array<int, 2> lineHull(P a, P b, vector<P>& poly) {
  ...
  if (sgn(a.cross(poly[endA], b)) < 0 || sgn(a.cross(poly[endB], b)) > 0)
    return {-1, -1};
  array<int, 2> res;
  for (int i = 0; i < (2); i++) {
    ...
    while ((lo + 1) % n != hi) {
      ...
      (sgn(a.cross(poly[m], b)) == sgn(a.cross(poly[endB], b)) ? lo : hi) = m;
    }
    res[i] = (lo + !sgn(a.cross(poly[hi], b))) % n;
    swap(endA, endB);
  }
  if (res[0] == res[1]) return {res[0], -1};
  if (!sgn(a.cross(poly[res[0]], b)) && !sgn(a.cross(poly[res[1]], b)))
