 ...
 * If strict is true, it returns false for points on the
 * boundary. The algorithm uses products in intermediate
 * steps so watch out for overflow. Time: O(n) Usage:
...
bool inPolygon(vector<P>& p, P a, bool strict = true) {
  ...
  for (int i = 0; i < (n); i++) {
    ...
    cnt ^=
      ((a.y < p[i].y) - (a.y < q.y)) * a.cross(p[i], q) >
      0;
  }
  return cnt;
}
