 ...
 * polygons (not necessarily convex). The points within
 * each polygon must be given in CCW order. (Epsilon checks
 * may optionally be added to sideOf/sgn, but shouldn't be
 * needed.) Time: $O(N^2)$, where $N$ is the total number
 * of points Status: stress-tested, Submitted on ECNA 2017
 * Problem A
 */
/**
 ...
 * towards $e$. 1/0/-1 $\Leftrightarrow$ left/on
 ...
 * watch out for overflow if using int or long long. Usage:
...
double polyUnion(vector<vector<P>>& poly) {
  ...
  for (int i = 0; i < ((int)(poly).size()); i++)
    for (int v = 0; v < ((int)(poly[i]).size()); v++) {
      P A = poly[i][v],
        B = poly[i][(v + 1) % (int)(poly[i]).size()];
      vector<pair<double, int>> segs = {{0, 0}, {1, 0}};
      for (int j = 0; j < ((int)(poly).size()); j++)
        if (i != j) {
          for (int u = 0; u < ((int)(poly[j]).size());
               u++) {
            P C = poly[j][u],
              D = poly[j][(u + 1) % (int)(poly[j]).size()];
            int sc = sideOf(A, B, C), sd = sideOf(A, B, D);
            if (sc != sd) {
              double sa = C.cross(D, A),
                     sb = C.cross(D, B);
              if (min(sc, sd) < 0)
