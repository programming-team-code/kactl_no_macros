 ...
 * circle with a ccw polygon. Time: O(n) Status: Tested on
 * GNYR 2019 Gerrymandering, stress-tested
 */
/**
...
double circlePoly(P c, double r, vector<P> ps) {
  auto tri = [&](P p, P q) {
    ...
    auto s = max(0., -a - sqrt(det)),
         t = min(1., -a + sqrt(det));
    if (t < 0 || 1 <= s)
      return atan2(p.cross(q), p.dot(q)) * r2;
    P u = p + d * s, v = p + d * t;
    return atan2(p.cross(u), p.dot(u)) * r2 +
      u.cross(v) / 2 + atan2(v.cross(q), v.dot(q)) * r2;
  };
  auto sum = 0.0;
  for (int i = 0; i < ((int)(ps).size()); i++)
    sum +=
      tri(ps[i] - c, ps[(i + 1) % (int)(ps).size()] - c);
  return sum;
}
