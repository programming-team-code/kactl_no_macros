 ...
 * returned. Should work for doubles as well, though there
 * may be precision issues in 'circ'. Returns triangles in
 * order \{t[0][0], t[0][1], t[0][2], t[1][0], \dots\}, all
 * counter-clockwise. Time: O(n \log n) Status:
 * stress-tested
 */
/**
...
P arb(LLONG_MAX,
  LLONG_MAX); // not equal to any other point
struct Quad {
  Q rot, o;
  P p = arb;
  bool mark;
...
bool circ(P p, P a, P b,
  P c) { // is p in the circumcircle?
...
pair<Q, Q> rec(const vector<P>& s) {
  ...
  tie(B, rb) =
    rec({(int)(s).size() - half + begin(s), end(s)});
  while (
    (B->p.cross(A->F(), A->p) < 0 && (A = A->next())) ||
  ...
  for (;;) {
    ...
    if ((LC->F().cross(base->F(), base->p) > 0))
      while (
        circ(LC->o->F(), base->F(), base->p, LC->F())) {
        Q t = LC->o;
        splice(LC, LC->prev());
    ...
    if ((RC->F().cross(base->F(), base->p) > 0))
      while (circ(RC->prev()->F(), base->F(), base->p,
        RC->F())) {
        Q t = RC->prev();
        splice(RC, RC->prev());
    ...
    if (!(LC->F().cross(base->F(), base->p) > 0) &&
      !(RC->F().cross(base->F(), base->p) > 0))
      break;
    if (!(LC->F().cross(base->F(), base->p) > 0) ||
      ((RC->F().cross(base->F(), base->p) > 0) &&
        circ(RC->F(), RC->p, LC->F(), LC->p)))
