...
struct Node {
  ...
  T distance(
    const P& p) { // min squared distance to a point
  ...
  Node(vector<P>&& vp): pt(vp[0]) {
    ...
    if (vp.size() > 1) {
      ...
      // (not best performance with many duplicates in the
      // middle)
...
struct KDTree {
  ...
  // distance (requires an arbitrary operator< for Point)
