 ...
 * but note that negative cost cycles are not supported. To
 * obtain the actual flow, look at positive values only.
 * Status: Tested on kattis:mincostmaxflow, stress-tested
 * against another implementation Time: $O(F E \log(V))$
 * where F is max flow. $O(VE)$ for setpi.
 */
...
struct MCMF {
  ...
  void addEdge(int from, int to, int64_t cap,
    ...
    if (from == to) return;
    ed[from].push_back(
      edge{from, to, (int)(ed[to]).size(), cap, cost, 0});
    ed[to].push_back(edge{
      to, from, (int)(ed[from]).size() - 1, 0, -cost, 0});
  }
  void path(int s) {
